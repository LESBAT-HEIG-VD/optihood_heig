...
class Building:
        ...
        for i, s in data.iterrows():
            if s["active"]:
                ...
                if temperatureLevels:
                    ...
                    # connectBuses = [self.__busDict[s["connect"] + '__' + self.__buildingLabel]]                    
    ...
    def _addGasBoiler(self, data, opt, dispatchMode, temperatureLevels):
        ...
        efficiency2 = float(data["efficiency"].split(",")[2])
        outputBuses.append(self.__busDict[outputSHBusLabel])
        outputBuses.append(self.__busDict[outputDHWBusLabel])
        efficiency = [efficiency1, efficiency2]
        if temperatureLevels:
            outputBusLabel3 = data["to"].split(",")[1] + '__' + self.__buildingLabel  # outputSHBusLabel, outputDHWBusLabel, outputBusLabel3 are in the order of increasing temperatures
            outputBuses.append(self.__busDict[outputBusLabel3])
            efficiency3 = float(data["efficiency"].split(",")[1])
